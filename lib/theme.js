...
library.init = async function (params) {
	...
	routeHelpers.setupAdminPageRoute(router, '/admin/plugins/theme-grinnellian', [], controllers.renderAdminPage);
...
library.addAdminNavigation = async function (header) {
	header.plugins.push({
		route: '/plugins/theme-grinnellian',
		icon: 'fa-paint-brush',
		name: 'Grinnellian Theme',
	});
	return header;
};
...
library.loadThemeConfig = async function (uid) {
	...
	config.numMobileNavItems = parseInt(config.numMobileNavItems, 10) || 4;
	return config;
};
...
library.filterMiddlewareRenderHeader = async function (hookData) {
	...
	const config = await library.loadThemeConfig(hookData.req.uid);
	if (hookData.templateData.navigation) {
		hookData.templateData.navigation.forEach((item, index) => {
			item.showInMobileBottomBar = index < config.numMobileNavItems;
		});
	}
